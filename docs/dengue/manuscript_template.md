...
**Pipeline-numbers timestamp (auto):** {{PIPELINE_RUN_TIMESTAMP}}
...
pipeline and applied it to the {{N_VACCINE_CONSTRUCTS}} parent strains of
...
composite correlated with published efficacy at Pearson r = {{R_PHASE3}}
(n={{N_PHASE3_GROUPS}}) and correctly ranked Butantan-DV first. We
...
**Phase 3 vaccine constructs.** {{N_VACCINE_CONSTRUCTS}} GenBank protein
...
DENV-4. Highest composite was {{TOP_SEROTYPE}} (Tier A+B = {{TOP_SEROTYPE_SCORE}});
lowest was {{BOTTOM_SEROTYPE}} (Tier A+B = {{BOTTOM_SEROTYPE_SCORE}}). The
spread (Δ = {{SEROTYPE_SCORE_RANGE}}) reflects the well-known asymmetry
between DENV-2 (canonical neutralization-target backbone of TAK-003) and
DENV-4 (smaller circulating diversity).

**Table 1.** Per-serotype Tier A and Tier B sub-scores ({{TABLE1_PATH}}).

### 3.2 HLA equity by region

**Figure 4** is the per-region IEDB population-coverage heatmap. Coverage
falls below 0.7 in {{N_REGIONS_BELOW_THRESHOLD}} of the 5 regions
({{REGIONS_BELOW_THRESHOLD}}), reproducing the equity gap that motivated
...
efficacy across {{N_VACCINE_CONSTRUCTS}} constructs grouped under the three
programs. Pearson r = {{R_PHASE3}} (95% CI {{R_PHASE3_CI}}, n = {{N_PHASE3_GROUPS}}
program-level points). The model rank-orders {{RANK_ORDER_VACCINES}}, which
{{RANK_ORDER_OUTCOME}} the published clinical rank order (Butantan-DV >
TAK-003 > CYD-TDV).
...
off-diagonal {{EDE_OFFDIAG_MEAN}} ± {{EDE_OFFDIAG_SD}} (mean ± SD across
...
The model does not capture {{FAILURE_MODE_1}}, the most likely reason for
{{FAILURE_VACCINE}}'s {{FAILURE_DIRECTION}} from predicted efficacy. This
...
Butantan-DV > TAK-003 > CYD-TDV rank order at Pearson r = {{R_PHASE3}}
(95% CI {{R_PHASE3_CI}}). The pipeline is open-source, containerised, and
...
*before* unblinding, so the empirical correlate can mature from r = {{R_PHASE3}}
