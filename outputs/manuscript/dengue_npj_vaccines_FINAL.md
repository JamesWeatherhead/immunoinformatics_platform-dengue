...
Here we ask whether sequence- and structure-only proxies of those four rows can be computed pre-Phase-1 to rank-order vaccine candidates. We forked Versiani/McCaffrey's published immunoinformatics pipeline and applied it to the 12 parent strains of the three Phase 3 vaccines. Restricting B-cell and T-cell scoring to the EDE epitope window (Rouvinski 2015 residues), the pipeline-derived equal-weight composite correlated with published efficacy at Pearson r = 0.35 (n=3 vaccine programs; bootstrap 95% CI [-1, +1]; permutation p = 0.83); the post-hoc-optimal Tier-A-heavy weighting (w_A = 0.75) raises the in-sample r to 0.35. The pipeline correctly ranked Butantan-DV first. We propose this framework, the *ICA-derived sequence proxy* (ICA-SP), as a candidate prioritisation tool, not a substitute for clinical evaluation.
...
Figure 4 is the per-region IEDB Population Coverage heatmap. Brazilian regional coverage falls below 0.7 across all four DENV serotypes (range 0.43-0.45 per `outputs/table4_dengue/hla_coverage_by_region.tsv`) when the AFND-derived frequencies are computed directly, identifying a population-coverage gap in the highest-burden Phase 3 trial population. The earlier (Lancet ID-rejected) version of this work cited 9 secondary references for an "HLA inequity" claim where none of the cited papers had measured HLA frequencies; the present implementation derives every coverage value from the 16 primary-measurement papers listed in `docs/dengue/paperclip_research/hla_primary_measurements.md` (combined N > 150,000 typed individuals across Brazil, Latin America/Caribbean, Southeast Asia, South Asia, and Sub-Saharan Africa). Benchmarked against IEDB-curated dengue class-I T-cell epitopes across the 12 HLA-I alleles in the panel, the NetMHCpan-4.1 strong-binder predictions (rank <= 2.0) achieve median per-allele recall = 0.65 and median per-allele precision = 0.06 (micro-averaged: recall = 0.64 on 370/582 IEDB-positive epitopes recovered; precision = 0.06 on 5,443 predicted 9-mers; Supplementary Table S5).
...
Per-tier ablation: Tier A alone r = 0.32; Tier B alone r = -0.17 (anti-correlated; the three vaccines have closely clustered T-cell coverage at 0.83-1.00 normalized). Combined composite r = 0.35 with equal weights, r = 0.35 with the post-hoc-optimal w_A = 0.75 sweep weight (this optimal weight is not held-out and we report both numbers; Supplementary Table S1, S4).
